...
class UcclTransferEngine:
    ...
    def batch_transfer_sync(
        ...
        logger.info(
            "UCCL batch_transfer_sync: session=%s conn_id=%s blocks=%d src_mr_id=%s",
            session_id,
            conn_id,
            len(src_addrs),
            src_mr_id,
        ...
        meta_list = []
        for dst_addr, length in zip(dst_addrs, lengths):
        ...
        mr_ids = [src_mr_id for _ in src_addrs]
        try:
            ok = self.endpoint.writev(
                conn_id, mr_ids, src_addrs, lengths, meta_list, len(src_addrs)
            )
        except Exception:
            ok = False
