...
class UcclKVManager(MooncakeKVManager):
    ...
    def _transfer_data(
        ...
        logger.info(
            "UCCL transfer: session=%s blocks=%d src_mr_id=%s dst_base_ptr=0x%x",
            mooncake_session_id,
            len(transfer_blocks),
            src_mr_id,
            dst_base_ptr,
            ...
            for i in range(len(prefill_kv_indices)):
                ...
                for token_slot_in_page in range(page_size):
                    ...
                    transfer_blocks.append(
                        (src_slice_addr, dst_slice_addr, heads_bytes_per_token_to_send)
                    )
    ...
    def send_aux(
        ...
        for i, dst_aux_ptr in enumerate(dst_aux_ptrs):
            ...
            transfer_blocks = [(src_addr, dst_addr, length)]
    ...
    def maybe_send_extra(
        ...
        dst_state_data_ptrs: list[int],
        dst_state_metas: list[bytes],
        executor: concurrent.futures.ThreadPoolExecutor,
    ):
        state_type = getattr(self.kv_args, "state_type", "none")

        logger.info(
            "UCCL maybe_send_extra: session=%s state_type=%s",
            req.mooncake_session_id,
            state_type,
        )
        if state_type == "mamba":
            ...
            return self._send_kvcache_generic(
                ...
                dst_data_metas=dst_state_metas,
        ...
        while True:
            try:
                ...
                for req in reqs_to_be_processed:
                    if not req.is_dummy:
                        logger.info(
                            "UCCL transfer_worker: room=%s session=%s dst=%s:%s kv_chunk=%s",
                            kv_chunk.room,
                            req.mooncake_session_id,
                            req.endpoint,
                            req.dst_port,
                        ...
                        if self.is_mla_backend or (
                            ...
                            ret = self.send_kvcache(
                                ...
                                target_rank_registration_info.dst_kv_metas,
                        ...
                        if kv_chunk.is_last:
                            if kv_chunk.state_indices is not None:
                                ...
                                self.maybe_send_extra(
                                    ...
                                    target_rank_registration_info.dst_state_metas,
                                    executor,
                                )
                            ...
                            ret = self.send_aux(
                                ...
                                target_rank_registration_info.dst_aux_metas,
                            )
