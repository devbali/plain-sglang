...
(* KaRaMeL-compatible multiplication via repeated addition *)
...
(* Lifted top-level recursive helper for would_violate_decode_deadlines (function 9) *)
let rec would_violate_decode_deadlines_aux
...
=
  ...
  else if index deadlines_us i <= threshold then true
  else would_violate_decode_deadlines_aux deadlines_us n_running threshold (i + 1)
...
=
  ...
  would_violate_decode_deadlines_aux running_deadlines_us n_running threshold 0
...
(* Lifted top-level recursive helpers for sort_prefill_queue (function 11) *)
let rec sort_prefill_queue_gen_range (n: Prims.int) (i: Prims.int) : Tot (seq Prims.int) (decreases (n - i)) =
  if i >= n then empty
  else cons i (sort_prefill_queue_gen_range n (i + 1))

let rec sort_prefill_queue_remove_elem (s: seq Prims.int) (v: Prims.int) : Tot (seq Prims.int) (decreases (length s)) =
  if length s = 0 then empty
  else
    let hd = head s in
    let tl = tail s in
    if hd = v then tl
    else cons hd (sort_prefill_queue_remove_elem tl v)

let rec sort_prefill_queue_find_min
  (s: seq Prims.int)
  (deadlines_us: seq Prims.int)
  ...
  (tau_us: Prims.int)
  (now_us: Prims.int)
  : Tot Prims.int (decreases (length s))
=
  if length s = 1 then head s
  else
    let hd = head s in
    let tl = tail s in
    let rest = sort_prefill_queue_find_min tl deadlines_us total_kvs fair_shares tau_us now_us in
    let key_hd = edf_fairshare_key (index deadlines_us hd) (index total_kvs hd) (index fair_shares hd) tau_us now_us in
    let key_rest = edf_fairshare_key (index deadlines_us rest) (index total_kvs rest) (index fair_shares rest) tau_us now_us in
    if edf_compare key_hd key_rest then hd else rest

let rec sort_prefill_queue_sort_aux
  (s: seq Prims.int)
  (acc: seq Prims.int)
  (deadlines_us: seq Prims.int)
  (total_kvs: seq Prims.int)
  ...
  (tau_us: Prims.int)
  (now_us: Prims.int)
  : Tot (seq Prims.int) (decreases (length s))
=
  if length s = 0 then acc
  else
    let m = sort_prefill_queue_find_min s deadlines_us total_kvs fair_shares tau_us now_us in
    sort_prefill_queue_sort_aux (sort_prefill_queue_remove_elem s m) (snoc acc m) deadlines_us total_kvs fair_shares tau_us now_us
...
let sort_prefill_queue
  ...
  : seq Prims.int
=
  sort_prefill_queue_sort_aux (sort_prefill_queue_gen_range n 0) empty deadlines_us total_kvs fair_shares tau_us now_us

(* Lifted top-level recursive helper for warn_overdue_deadlines (function 12) *)
let rec warn_overdue_deadlines_aux
  (deadlines_us: seq Prims.int)
  (n: Prims.int)
  (now_us: Prims.int)
  (i: Prims.int)
...
=
  if i >= n then 0
  else (if index deadlines_us i <= now_us then 1 else 0) + warn_overdue_deadlines_aux deadlines_us n now_us (i + 1)
...
let warn_overdue_deadlines
  ...
  : Prims.int
=
  warn_overdue_deadlines_aux deadlines_us n now_us 0

(* Lifted top-level recursive helper for count_prefill_admitted (function 13) *)
let rec count_prefill_admitted_aux
  (total_kvs: seq Prims.int)
  (fair_shares: seq Prims.int)
  (n: Prims.int)
  (i: Prims.int)
...
=
  if i >= n then 0
  else (if index total_kvs i <= index fair_shares i then 1 else 0) + count_prefill_admitted_aux total_kvs fair_shares n (i + 1)
...
let count_prefill_admitted
  ...
  : Prims.int
=
  count_prefill_admitted_aux total_kvs fair_shares n 0

(* Helper: check_decode_deadlines_aux — build seq of 0/1 flags per deadline *)
...
(* Lifted top-level recursive helper for compute_global_slack (function 17) *)
let rec compute_global_slack_sum_aux
  (total_kvs: seq Prims.int)
  (n: Prims.int)
  (i: Prims.int)
  : Prims.int
=
  if i >= n then 0
  else index total_kvs i + compute_global_slack_sum_aux total_kvs n (i + 1)
...
let compute_global_slack
  ...
  : Prims.int
=
  let sum_kv = compute_global_slack_sum_aux total_kvs n 0 in
...
let isolated_prefill_latency
  ...
  : Prims.int
=
  let term1 = coeffs.cp_ns in
  let term2 = mul coeffs.alpha_p_nspt prompt_len 0 in
  let term3 = mul coeffs.beta_p_nspt max_tok 0 in
  let term4 = mul coeffs.gamma_p_nspt req_count 0 in
  let inner = term1 + term2 + term3 + term4 in
  let result = mul num_clients inner 0 in
  result / 1000
...
let isolated_decode_latency
  ...
  : Prims.int
=
  let term1 = coeffs.cd_ns in
  let term2 = mul coeffs.alpha_d_nspt sum_tok 0 in
  let term3 = mul coeffs.beta_d_nspt max_tok 0 in
  let term4 = mul coeffs.gamma_d_nspt req_count 0 in
  let inner = term1 + term2 + term3 + term4 in
  let result = mul num_clients inner 0 in
  result / 1000
...
let split_tau (tau_us: Prims.int) (cache_ratio_bps: Prims.int) (tau_decode_fixed_us: Prims.int) : tau_breakdown =
  let tau_cache = mul tau_us cache_ratio_bps 0 / 10000 in
  {
    tau_us = tau_us;
